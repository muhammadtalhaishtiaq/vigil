...
logger = logging.getLogger("macro-watchdog")

app = FastAPI(title="Macro Watchdog", version="1.0.0")
...
<title>Macro Watchdog — Vigil Intelligence</title>
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header {
    background: linear-gradient(135deg, #0d0a00 0%, #161100 100%);
  ...
  .btn-submit {
    ...
    color: #0d0a00; font-size: 1rem; font-weight: 800;
  ...
  .badge-running { background: #2a1d00; color: var(--accent); animation: pulse 1.5s infinite; }
  ...
  #output-box h3 {
    font-size: 0.9rem; margin: 16px 0 8px; color: #fcd34d;
  }
  #output-box h2:first-child { margin-top: 0; }
  #output-box strong { color: #fcd34d; }
...
<header>
  <div class="logo">🐕</div>
  <div>
    <h1>Macro Watchdog <span>by Vigil</span></h1>
  </div>
</header>
...
<div class="container">
  ...
  <div class="panel">
    ...
    <button class="btn-submit" id="submit-btn" onclick="analyze()">
      <span id="btn-text">🐕 Run Macro Watchdog</span>
    </button>
  </div>
  ...
  <div class="panel output-panel">
    <div class="output-header">
      <h2 style="margin:0; color: var(--accent); font-size:1rem;">📊 Macro Risk Profile</h2>
      <span id="status-badge" class="badge-idle">IDLE</span>
    </div>
    <div id="output-box" class="empty">
      <div class="empty-icon">🐕</div>
      <div>Fill in context + paste Signal Harvest report, then hit <strong>Run Macro Watchdog</strong></div>
    </div>
  </div>
...
<script>
  ...
  async function analyze() {
    ...
    try {
      ...
      btn.disabled = false;
      document.getElementById("btn-text").textContent = "🐕 Run Macro Watchdog";
    }
  }
</script>
</body>
...
async def analyze(request: Request):
    ...
    logger.info(f"Macro Watchdog: sector={sector}, horizon={risk_horizon}")
    try:
        from agent import run_macro_watchdog
        report = run_macro_watchdog(signal_report, business_type, sector, risk_horizon)
        logger.info("Macro Watchdog complete")
        return {"report": report}
    except Exception as e:
        logger.error(f"Macro Watchdog error: {e}")
        return JSONResponse(status_code=500, content={"detail": str(e)})


@app.get("/health")
async def health():
    return {"status": "ok", "agent": "macro-watchdog"}
