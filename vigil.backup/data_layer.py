...
def get_live_headlines(sector: str = "finance") -> list[dict]:
    ...
    _FALLBACK = [
        {"title": "Markets steady as investors await Fed guidance on rate path",
         "source": "Reuters", "url": "https://reuters.com", "publishedAt": now, "sentiment": "neutral"},
        {"title": "Inflation data shows cooling trend for third consecutive month",
         "source": "Bloomberg", "url": "https://bloomberg.com", "publishedAt": now, "sentiment": "positive"},
        {"title": "Tech sector faces headwinds amid rising Treasury yield concerns",
         "source": "FT", "url": "https://ft.com", "publishedAt": now, "sentiment": "negative"},
        {"title": "Global equity markets mixed ahead of key central bank decisions",
         "source": "CNBC", "url": "https://cnbc.com", "publishedAt": now, "sentiment": "neutral"},
        {"title": "Dollar strengthens as safe-haven demand picks up on geopolitical risk",
         "source": "WSJ", "url": "https://wsj.com", "publishedAt": now, "sentiment": "negative"},
        {"title": "AI and semiconductor stocks rally on strong earnings beat expectations",
         "source": "Axios", "url": "https://axios.com", "publishedAt": now, "sentiment": "positive"},
    ]

    # ── Try Yahoo Finance RSS (no API key required) ──────────────────────
    try:
        import xml.etree.ElementTree as ET
        rss_url = "https://finance.yahoo.com/news/rssindex"
        resp = requests.get(rss_url, timeout=6, headers={"User-Agent": "Vigil/2.0"})
        if resp.status_code == 200:
            ...
            for item in items[:8]:
                ...
                source_el = item.find("{http://purl.org/rss/1.0/modules/content/}encoded") or item.find("source")
                source = "Yahoo Finance"
                if source_el is not None and source_el.text:
                    source = source_el.text.strip()[:30]
                pub = item.findtext("pubDate") or now
            ...
            if len(results) >= 3:
                logger.info("Yahoo Finance RSS: %d headlines loaded", len(results))
                return results[:8]
    except Exception as exc:
        logger.debug("Yahoo Finance RSS failed: %s", exc)
        ...
        try:
            ...
            for art in articles[:8]:
                ...
                results.append({
                    ...
                    "source": source,
                    "url": art.get("url") or "",
                    "publishedAt": art.get("publishedAt") or now,
                    "sentiment": _classify_sentiment(title),
                })
            logger.info("NewsAPI: %d articles for sector '%s'", len(results), sector)
            return results if results else _FALLBACK
        except Exception as exc:
            logger.error("NewsAPI failed: %s", exc)

    logger.warning("Using static fallback headlines (no live source available)")
    return _FALLBACK
