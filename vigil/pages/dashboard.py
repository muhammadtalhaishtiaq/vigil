"""
pages/dashboard.py — Vigil Financial Intelligence Dashboard
...
function fc(text) {
  // Search in current document first (works in both standalone and iframe contexts)
  var inp = document.querySelector('[data-testid="stTextInput"] input');
...
function send() {
  ...
  // Fill Streamlit hidden text input — use document (not window.parent) to work inside iframes
  ...
  setTimeout(function() {
    var submitBtn = document.querySelector('[data-testid="stFormSubmitButton"] button');
  ...
  <div class="bubble" style="color:var(--tx2);font-size:11px;">
    ...
    Ask me anything to get started.
        ...
        for msg in history:
            ...
            escaped = _html.escape(str(content))
            if role == "user":
                msgs_html += f"""
<div class="msg user">
...
<div class="acard">
  ...
  <div class="acard-body">{_html.escape(ag_out[:600])}</div>
</div>"""

                disp = escaped.replace("&#x0a;", "<br>").replace("\n", "<br>")
                msgs_html += f"""
<div class="msg sys">
  <div class="mlbl">⚡ Vigil · {_html.escape(agent_label)}</div>
  <div class="bubble" style="font-size:11px;">{disp}{acard_html}</div>
</div>"""
  ...
  <div class="mbody">
    ...
    with st.container():
        ...
        with st.form("chat_form", clear_on_submit=True):
            msg_input = st.text_input("Chat input", key="chat_input", label_visibility="collapsed")
