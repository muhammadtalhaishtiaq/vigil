...
def _persist_current_session() -> None:
    """Write current st.session_state vigil keys to disk for cross-reload persistence."""
    sid = st.session_state.get("vigil_session_id")
    if not sid:
        return
    try:
        from storage import persist_session
        persist_session(sid, dict(st.session_state))
...
def init_conversation() -> None:
    ...
    Persistence strategy:
        1. Check URL query param ``?sid=<uuid>`` for a returning session.
        2. If found and a matching session file exists, restore all keys.
        3. Otherwise create fresh state, generate a new session_id, and
           write ``?sid=<uuid>`` into the URL for future reloads.
    ...
    """
    from storage import restore_session, session_exists, persist_session

    # ── Step 1: already initialised this Streamlit session (mid-session rerun) ──
    if "vigil_session_id" in st.session_state:
        # Always ensure URL param is set so page reloads can restore the session
        try:
            current_sid = st.session_state["vigil_session_id"]
            if st.query_params.get("sid") != current_sid:
                st.query_params["sid"] = current_sid
        except Exception:
            pass
        return

    # ── Step 2: returning user — check URL for session ID ──
    try:
        url_sid = st.query_params.get("sid", "")
    except Exception:
        url_sid = ""

    if url_sid and session_exists(url_sid):
        saved = restore_session(url_sid)
        if saved:
            # Re-hydrate st.session_state from persisted file
            for k, v in saved.items():
                if k != "_persisted_at" and k not in st.session_state:
                    st.session_state[k] = v
            # Ensure agent dicts are present (in case schema grew)
            st.session_state.setdefault("agent_statuses", {a: "idle" for a in _ALL_AGENTS})
            st.session_state.setdefault("agent_elapsed", {a: None for a in _ALL_AGENTS})
            logger.info("Session restored from file: %s", url_sid)
            return

    # ── Step 3: brand new session ──
    new_sid = str(uuid.uuid4())
    defaults: dict = {
        "vigil_conversation": [],
        "vigil_session_id": new_sid,
    ...
    # Persist new session immediately and set URL param for future reloads
    try:
        persist_session(new_sid, dict(st.session_state))
        st.query_params["sid"] = new_sid
    except Exception as exc:  # noqa: BLE001
        logger.warning("Could not set session URL param: %s", exc)

    logger.info("New session created: %s", new_sid)
